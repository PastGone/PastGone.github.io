...
$(function () {
  const emptyInputMsg = "复制 JSON 到这里";
...
function constructJSONErrorHTML(rawErrorMessage, errorIndex, json) {
  var errorHeading =
    '<p><span class="clr-red">' + rawErrorMessage + "</span><p>";
  var markedPart =
    '<span class="json-go-faulty-char">' + json[errorIndex] + "</span>";
  var markedJsonString = [
    json.slice(0, errorIndex),
    markedPart,
    json.slice(errorIndex + 1),
  ].join("");
  ...
  for (var i = 0; i < jsonStringLines.length; i++) {
    if (jsonStringLines[i].indexOf('<span class="json-go-faulty-char">') > -1)
      // faulty line
      var wrappedLine =
        '<div class="faulty-line">' + jsonStringLines[i] + "</div>";
    else var wrappedLine = "<div>" + jsonStringLines[i] + "</div>";
